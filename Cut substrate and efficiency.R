...
ggplot(dat, aes(x=`artifact size group (cm)`, y=Time_s, color=`Profile Curvature`, shape = `Profile Curvature`)) + facet_wrap(~material, ncol = 2, scales = 'free') +
  ...
  theme(plot.title = element_text(size = 18, face="bold", hjust = 0.5), 
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) + 
  labs(x="Artifact size (cm)", y = "Time (s)")
...
glm_fit_size_plan <- glmer(Time_s ~ `artifact size group (cm)`*material*`Plan index` + (1|participant), data = dat, family = "poisson")
a <- summary(glm_fit_size_plan)
write.csv(round(a$coefficients,2), "glm_size_plan.csv")

glm_fit_size_profile <- glmer(Time_s ~ `artifact size group (cm)`*material*`profile index` + (1|participant), data = dat, family = "poisson")
a <- summary(glm_fit_size_profile)
write.csv(round(a$coefficients, 2), "glm_size_profile.csv")


glm_fit_edgelength_plan <- glmer(Time_s ~ sEdge_length_mm*material*`Plan index` + (1|participant), data = dat, family = "poisson")
a <- summary(glm_fit_edgelength_plan)
write.csv(round(a$coefficients, 2), "glm_edgelength_plan.csv")

glm_fit_edgelength_profile <- glmer(Time_s ~ sEdge_length_mm*material*`profile index` + (1|participant), data = dat, family = "poisson")
